...
const getAllFormFields = (root: Document | ShadowRoot = document): FormField[] => {
    ...
    allPotentialInputs.forEach((el) => {
        ...
        // Generate a unique ID if none exists and persist it to the DOM
        let fieldId = element.id || element.name || element.getAttribute('data-field-id');
        if (!fieldId) {
            fieldId = `generated_id_${Math.random().toString(36).substr(2, 9)}`;
...
const findResumeUploadField = (): HTMLInputElement | null => {
    // 1. Ashby Specific Check: They use this specific ID often
...
const injectResumeFile = (resumeBlob: string, fileName: string, mimeType: string = 'application/pdf'): boolean => {
    ...
    try {
        ...
        // --- CRITICAL FIX: React Prototype Hack ---
        // React overrides the standard 'files' setter. We must get the native setter
        // from the HTMLInputElement prototype and call it directly.
        ...
        // Dispatch events in this specific order for React to pick it up
        input.dispatchEvent(new Event('click', { bubbles: true })); // Wakes up some listeners
        input.dispatchEvent(new Event('change', { bubbles: true }));
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('blur', { bubbles: true }));
...
const scanPage = () => {
    ...
    if (fields.length > 0) {
        ...
        console.warn(`[ContentScript] ⚠️ No fields detected! This might be a dynamic form.`);
    }

    return { context, fields };
};

// --- NEW HELPER: For Ashby "Location" Comboboxes ---
const fillAshbyLocation = async (element: HTMLInputElement, value: string) => {
    try {
        console.log(`[Job-Agent] Filling Ashby location: ${value}`);
        element.focus();

        // 1. Set text to trigger search (using native setter)
        const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value")?.set;
        if (nativeSetter) {
        ...
        // 3. Wait for dropdown (Crucial!)
        await new Promise(r => setTimeout(r, 800));

        // 4. Select first result (Down Arrow -> Enter)
...
if (typeof chrome !== 'undefined' && chrome.runtime) {
    ...
    // MutationObserver to detect lazy-loaded forms (Ashby, Greenhouse)
    ...
    const startObserving = () => {
        ...
        observer = new MutationObserver((mutations) => {
            ...
            if (hasNewNodes) {
                ...
                if (currentInputs !== lastFieldCount) {
                    console.log(`[ContentScript] DOM Mutation detected new inputs: ${currentInputs}`);
                    lastFieldCount = currentInputs;
                    // We don't auto-send message to avoid spamming, but we ensure next GET_PAGE_CONTEXT is fresh
                }
            }
        });
    ...
    // Robust Polling Scanner
    const scanWithRetry = async (maxAttempts = 10, interval = 500): Promise<{ context: any, fields: FormField[] }> => {
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            ...
            // If no fields, but we are on a known ATS domain, wait and retry
            const isATS = window.location.href.includes('ashby') ||
                window.location.href.includes('greenhouse') ||
                window.location.href.includes('lever') ||
                window.location.href.includes('workday');

            if (!isATS && attempts > 1) {
                // If not a known ATS, give up earlier
                break;
            }

            console.log(`[ContentScript] Attempt ${attempts + 1}/${maxAttempts}: No fields found. Retrying in ${interval}ms...`);
    ...
    else window.addEventListener('DOMContentLoaded', startObserving);

    chrome.runtime.onMessage.addListener((request: any, sender: any, sendResponse: any) => {
        if (request.type === 'GET_PAGE_CONTEXT') {
            // Use the robust scanner
            scanWithRetry().then(({ context, fields }) => {
                if (fields.length > 0) {
                    console.log(`[ContentScript] Scan complete: Found ${fields.length} fields.`);
                } else {
                    console.warn(`[ContentScript] Scan complete: Still 0 fields after retries.`);
                }
                sendResponse({ success: true, context, fields });
            });
            return true; // Keep channel open
